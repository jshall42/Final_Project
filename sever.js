...
app.post('/peerreview/assessments', (req, res) => 
{
    ...
    let strCourseCode = req.body.CourseCode
    ...
    let arrParameters = [strCourseCode, strStartDate, strEndDate, strName, strStatus, strType]

    const Insert = `INSERT INTO tblAssessments (CourseCode, StartDate, EndDate, Name, Status, Type) VALUES (?, ?, ?, ?, ?, ?)`;
...
{
    let strCourseCode = req.params.CourseCode;

    let Select = `SELECT * FROM tblAssessments WHERE CourseCode = ?`

    db.all(Select,[strCourseCode],function(err,result){
        if(err)
        {
            console.log(err)
            res.status(500).json
