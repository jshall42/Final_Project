...
app.post('/peerreview/assessments', (req, res) => 
{
    ...
    let intCourseID = req.body.CourseID
    ...
    let arrParameters = [intCourseID, strStartDate, strEndDate, strName, strStatus, strType]
...
{
    let intCourseID = req.params.CourseID;

    let Select = `SELECT * FROM tblAssessments WHERE CourseID = ?`

    db.all(Select,[intCourseID],function(err,result){
        if(err)
        {
            console.log(err)
            res.status(500).json
